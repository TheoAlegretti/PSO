...
def actualisation_position(store_dict, params,fct):
    """
    ...
    store_dict["updated_pso"]["inputs"] = np.add(
        store_dict["updated_pso"]["inputs"], store_dict["velocity"]
    )

    # contrainte de minx maxx
    out_range_index = np.where(
        (store_dict["updated_pso"]["inputs"] > params["max_x"])
        | (store_dict["updated_pso"]["inputs"] < params["min_x"])
    )
    store_dict["updated_pso"]["inputs"][:, out_range_index] = np.random.uniform(
        ...
        size=(
            params["Dim"],
            len(store_dict["updated_pso"]["inputs"][out_range_index]),
        ),
    )
    ...
    if params["min_max"] == "min":
        update_index = np.where(
            ...
            > store_dict["updated_pso"]["output"]
        )
        store_dict["best_personal_output"]["output"] = np.minimum(
            store_dict["best_personal_output"]["output"],
            store_dict["updated_pso"]["output"],
        )
    else:
        store_dict["best_personal_output"]["output"] = np.maximum(
            store_dict["best_personal_output"]["output"],
            store_dict["updated_pso"]["output"],
        )
        update_index = np.where(
    ...
    store_dict["best_personal_output"]["inputs"][:, update_index] = store_dict[
        "updated_pso"
    ]["inputs"][:, update_index]

    store_dict["best_bird"]["output"] = store_dict["best_personal_output"][
        "output"
    ][arg_min_max(store_dict["best_personal_output"]["output"], params["min_max"])]
    store_dict["best_bird"]["input"] = store_dict["best_personal_output"]["inputs"][
        :,
        arg_min_max(
            store_dict["best_personal_output"]["output"], params["min_max"]
        ),
    ]
...
def actualisation_vitesse(store_dict, params) -> dict:
    ...
    store_dict["velocity"] = (
        ...
        * (
            np.subtract(
                store_dict["updated_pso"]["inputs"],
                store_dict["best_bird"]["input"].reshape(2, 1),
            )
        )
        + params["c2"]
        * random.random()
        * (
            np.subtract(
                store_dict["updated_pso"]["inputs"],
                store_dict["best_personal_output"]["inputs"],
            )
        )
    )
    return store_dict
...
def initialization_pos_vit(fct, params):
    ...
    output = fct(inputs)
    best_bird = {
        "output": output[arg_min_max(output, params["min_max"])],
        "input": inputs[:, arg_min_max(output, params["min_max"])],
    }
    return {
        "updated_pso": {"inputs": inputs, "output": output},
...
def iteration_loop(fct, params):
    ...
    for iteration in range(1, params["max_ite"]):
        ...
        store_dict = actualisation_position(store_dict, params,fct)
    return store_dict


def storing_and_display(display,best_of_info,params): 
...
def pso_LM_v1(fct, params):
    ...
    best_of_info = storing_and_display(False,best_of_info,params)
...
def pso_LM_v3(fct, params):
    ...
    # Algo chained : TO DO : Mettre des @numba boucle ou opti avec parallélisation
    with concurrent.futures.ProcessPoolExecutor() as executor:  # Add parentheses here
        futures = [
...
def pso_LM_v2(fct, params):
    ...
    # Algo chained : TO DO : Mettre des @numba boucle ou opti avec parallélisation
    with concurrent.futures.ThreadPoolExecutor() as executor:  # Add parentheses here
        futures = [
            executor.submit(iteration_loop, fct, params)
            for _ in range(params["nb_simulation_MC"])
        ]
